...
- `RCLONE_MANAGER_GUI_RC_USER`: username đăng nhập rclone RC/web GUI.
- `RCLONE_MANAGER_GUI_RC_PASS`: password đăng nhập rclone RC/web GUI. Nên đặt riêng, không dùng lại hash Caddy.
...
- Nên đặt thêm `RCLONE_MANAGER_GUI_RC_USER` và `RCLONE_MANAGER_GUI_RC_PASS` vì host port có thể truy cập trực tiếp qua LAN/Tailscale.
