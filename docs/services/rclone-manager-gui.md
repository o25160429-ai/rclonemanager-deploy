...
- `RCLONE_MANAGER_GUI_RC_USER`: username auth riêng của rclone RC/web GUI. Bắt buộc khi `RCLONE_MANAGER_GUI_ENABLED=true`.
- `RCLONE_MANAGER_GUI_RC_PASS`: password auth riêng của rclone RC/web GUI. Bắt buộc khi `RCLONE_MANAGER_GUI_ENABLED=true`.
...
## Bảo mật
- Public route `rclone-gui` không dùng Caddy Basic Auth; auth nằm trong chính `rclone rcd` qua `RCLONE_MANAGER_GUI_RC_USER` và `RCLONE_MANAGER_GUI_RC_PASS`.
- Caddy vẫn reverse proxy hostname `rclone-gui`, nhưng không gắn label `caddy.basic_auth` cho service này để tránh prompt lặp giữa Caddy và WebUI.
- Nếu publish host port ra LAN/Tailscale, route đó cũng dùng cùng auth của rclone GUI.
