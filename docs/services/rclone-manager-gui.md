...
- `RCLONE_MANAGER_GUI_RC_USER`: username auth riêng của rclone RC/web GUI. Để trống cùng `RCLONE_MANAGER_GUI_RC_PASS` để tắt auth rclone.
...
- Auth riêng của `rclone rcd` là optional: để trống `RCLONE_MANAGER_GUI_RC_USER` và `RCLONE_MANAGER_GUI_RC_PASS` thì không thêm `--rc-user/--rc-pass`; điền cả hai thì GUI yêu cầu thêm lớp đăng nhập rclone.
