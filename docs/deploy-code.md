...
- `start`, `stop`, `restart` hỗ trợ cả `services` và `containers`.
- `rebuild` chỉ áp dụng với Compose service. Nếu truyền container thuộc Compose, sidecar cố gắng suy ra `com.docker.compose.service` rồi rebuild service đó.
