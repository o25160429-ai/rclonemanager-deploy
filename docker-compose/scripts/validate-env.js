...
checkOptional("RCLONE_MANAGER_REQUIRE_GOOGLE_AUTH", "true|false toggle for Firebase Google auth", (v) => (isBool(v) ? null : "must be true|false"));
...
checkOptional("DOCKER_DEPLOY_CODE_API_TOKEN", "optional sidecar API token", (v) => (v.length >= 16 ? null : "should be at least 16 characters"));
...
const hasServiceAccount = Boolean(
  (env.RCLONE_MANAGER_FIREBASE_SERVICE_ACCOUNT_JSON || "").trim() || (env.RCLONE_MANAGER_FIREBASE_SERVICE_ACCOUNT_PATH || "").trim(),
);
...
for (const key of [
  "ENABLE_DOZZLE",
  "ENABLE_FILEBROWSER",
  "RCLONE_MANAGER_GUI_ENABLED",
  "ENABLE_WEBSSH",
  "ENABLE_TAILSCALE",
  "DOCKER_DEPLOY_CODE_ENABLED",
  "DOCKER_DEPLOY_CODE_APP_PROXY_ENABLED",
